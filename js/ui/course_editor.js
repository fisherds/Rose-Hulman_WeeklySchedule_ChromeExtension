 ...
 *
 * @param {rosegrid.model.Course=} course the course that will be edited.
 * This can be set during initialization or set later, but must be set before the dialog is shown.
 * @constructor
 */
rosegrid.ui.CourseEditor = function(course) {
   ...
   */
  this.course_ = course || null;

  /**
   * Holds the properties of the course during the editing process.
   * For simplicity it is a rosegrid.model.Course, but it has no week model object.
   * Really I should make a rosegrid.model.CourseProperties class.  The properties of the course_
   * get copied into this object at the start of editing and the finished properties are copied back
   * if the user hits OK.
   * @type {rosegrid.model.Course}
   */
...
/**
 * Displays the course editor dialog box with the data for the given course.  If a user hits ok
 * the course is modified and an event is fired.
 * 
 ...
 */
rosegrid.ui.CourseEditor.prototype.launchEditorForCourse = function(course, isNewCourse) {
  ...
  this.courseDialog_.setContent(rosegrid.templates.courseDialog.dialogTabBar(data));
...
rosegrid.ui.CourseEditor.prototype.handleTabSelection_ = function(e) {
    ...
    if (tabSelected.getCaption() == 'Course') {
      goog.soy.renderElement(contentElement, rosegrid.templates.courseDialog.courseTab, this.course_);
      return;
    }
